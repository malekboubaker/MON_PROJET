...
void displayCarList(struct Voiture voitures[], int numOfCars) {
    printf("\n Liste des voitures :\n");

    for (int i = 0; i < numOfCars; i++) {
        printf("Matricule: %.0lf\tModele: %s\tMarque: %s\tÉtat: %c\tRemarque: %s\n",
               voitures[i].matricule, voitures[i].modele, voitures[i].marque,
               voitures[i].etat, voitures[i].remarque);
    }
...
int main() {
    ...
    FILE *file2 = fopen(HISTORIQUE_FILE, "a+");
    if (file2 == NULL) {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2:
                // Display all cars
                displayCarList(voitures, numOfCars);
                break;
            case 3:
                // Update a car
                ...
                if (!found) {
                    printf("Aucune voiture avec ce matricule.\n");
                }
                break;
            case 4:
                // Delete a car
